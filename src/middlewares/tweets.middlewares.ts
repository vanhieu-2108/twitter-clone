      ...
      custom: {
        options: async (value, { req }) => {
          ...
          if (
            ...
            throw new Error(TWEET_MESSAGES.CONTENT_MUST_NOT_BE_A_NON_EMPTY_STRING)
          }
...
// Nếu không dùng try catch thì phải dùng wrapHandler

export const audienceValidation = wrapHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (tweet.audience === TweetAudience.TwitterCircle) {
    ...
    // Kiểm tra tài khoản tác giả có bị block hay không
    ...
    // Nếu bạn không phải là tác giả và không nằm trong Twitter Circle của tác giả thì quăng lỗi
